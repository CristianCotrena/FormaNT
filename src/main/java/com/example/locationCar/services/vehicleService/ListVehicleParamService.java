...
public class ListVehicleParamService {
    ...
    public BaseDto listVehicles(Pageable pageable, String color, Double rating, Double max, Double min) {
        List<BaseErrorDto> errors = new ArrayList<>();
        Page<VehicleModel> vehicles = vehicleRepository.findAll(buildSpec(color, rating, max, min), pageable);
        ...
        List<VehicleRecordDto> vehicleDtoList = vehicles.getContent().stream().map(v -> new VehicleRecordDto(v)).collect(Collectors.toList());
        Page<VehicleRecordDto> vehicleDtoPage = new PageImpl<>(vehicleDtoList, pageable, vehicles.getTotalElements());

        return new ResponseSuccessBuilder<Page<VehicleRecordDto>>(HttpStatus.OK, vehicleDtoPage, SuccessMessage.LIST_VEHICLES).get();
    }

    private Specification<VehicleModel> buildSpec(String color, Double rating, Double max, Double min){
        Specification<VehicleModel> spec = Specification
                .where(VehicleSpecifications.colorEquals(color))
